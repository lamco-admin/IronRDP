...
pub fn rdpemt_pdu_decode(data: &[u8]) {
    ...
    use ironrdp_rdpemt::pdu::{
        MultitransportRequest, MultitransportResponse, TunnelCreateRequest, TunnelCreateResponse, TunnelData,
    ...
    match selector % 8 {
        ...
        6 => drop(decode::<MultitransportRequest>(payload)),
        _ => drop(decode::<MultitransportResponse>(payload)),
    }
}
...
///
/// [MS-RDPEUDP] 2.2.2.7 caps the ACK vector at 2048 elements, and the elements
/// are run-length coded, so a short datagram can describe a very long run. The
/// crash-only decode target only notices if that allocation aborts the process;
/// this one asserts the decoder honours the cap before it allocates, which is
/// the difference between rejecting a hostile datagram and being killed by the
/// OOM killer.
pub fn rdpeudp_ack_vector(data: &[u8]) {
    ...
    /// The cap from 2.2.2.7. A decoded vector longer than this means the
    /// bound was not enforced on the decode path.
    const MAX_ACK_VECTOR: usize = 2048;

    if let Ok(payload) = decode::<AckVectorPayload>(data) {
        assert!(
            payload.entries.len() <= MAX_ACK_VECTOR,
            "decoded {} ACK vector entries, above the 2048 cap",
            payload.entries.len()
        );
    ...
    if let Ok(header) = decode::<V1AckVectorHeader>(data) {
        assert!(
            header.elements.len() <= MAX_ACK_VECTOR,
